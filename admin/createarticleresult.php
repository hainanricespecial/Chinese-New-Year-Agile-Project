            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        /* Get the date using. POST */
                        $insertDate = $_POST['date'];
                        $articleTitleSanitized = htmlspecialchars($_POST['articleTitle']);
                        $articleContentSanitized = htmlspecialchars($_POST['articleContent']);

                        /* SQL command to insert into database */
                        $insertCommand = mysqli_prepare($connection, "INSERT INTO articles(article_title, article_content, article_created_date, article_last_modified_date)
                        VALUES (?, ?, ?, ?)");
                        $insertCommand->bind_param('ssss', $articleTitleSanitized, $articleContentSanitized, $insertDate, $insertDate);
                        $execute = mysqli_stmt_execute($insertCommand);

                        if ($execute == true) 
                        {
                            echo "<h2> Your article has been created successfully. </h2>";
